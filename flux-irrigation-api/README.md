...
- 🔀 **Multi-Probe Conflict Resolution** — When multiple probes monitor the same zone, choose how readings are combined: Conservative (wettest probe wins, skip if any saturated), Average (blend all readings, skip if majority saturated), or Optimistic (driest probe wins, skip only if all saturated) — configurable in Moisture Probe settings
...
- 💧 **Water Source & Cost** — Configure your water source (City Water, Reclaimed Water, or Well Water) via the ⚙️ settings gear on the Estimated Gallons card; for City or Reclaimed water, enter your cost per 1,000 gallons to see estimated water costs displayed on the card and in PDF reports; Well Water is auto-detected when a Pump Start Relay zone is configured (no water utility cost — pump electricity costs are tracked separately in the Pump Monitor card)
