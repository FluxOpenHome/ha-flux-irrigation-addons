...
- 🔩 **Zone head details** — Click the ℹ icon on any zone to document every sprinkler head in the zone: head type (pop-up spray, rotary, gear rotor, impact, micro-spray, bubbler, drip, strip), brand & model, mount type, GPM, spray arc, throw radius, pop-up height, PSI, and a name/location for each head; total zone GPM is calculated automatically; helps professionals service your system
...
- 🔩 **Remote zone head details** — View and edit sprinkler head inventory on customer zones: head type, brand, model, mount, GPM, spray arc, radius, pop-up height, PSI, and location notes; total zone flow calculated automatically for professional service documentation
