    ...
    <div id="detailView" class="detail-view">
        ...
        <div id="detailNotesSection" style="margin-bottom:16px;display:none;">
            <div id="detailNotesDisplay" style="display:flex;align-items:flex-start;gap:8px;">
                <button class="btn btn-secondary btn-sm" onclick="editDetailNotes()" style="flex-shrink:0;">Edit</button>
                <div style="flex:1;">
        ...
        <div class="card" id="detailIssuesCard" style="display:none;">
            <div class="card-header" onclick="toggleCard('issues')" style="cursor:pointer;user-select:none;">
                <h2 style="display:flex;align-items:center;gap:8px;"><span id="cardChevron_issues" style="font-size:12px;transition:transform 0.2s;display:inline-block;transform:rotate(90deg);">&#9654;</span> &#9888;&#65039; Issues <span id="detailIssuesBadge" style="font-size:12px;padding:2px 8px;border-radius:10px;background:#e74c3c;color:white;margin-left:6px;display:none;"></span></h2>
                <div style="display:flex;align-items:center;gap:8px;">
                    <a href="#" onclick="lockCard('issues',event)" id="cardLock_issues" style="font-size:14px;text-decoration:none;" title="Lock open">&#128275;</a>
                </div>
            </div>
            <div class="card-body" id="cardBody_issues">
            ...
            <div class="card" id="detailEstGallonsCard" style="display:none;">
                <div class="card-header" onclick="toggleCard('gallons')" style="cursor:pointer;user-select:none;">
                    ...
                    <div style="display:flex;gap:6px;align-items:center;">
                        ...
                        <select id="mgmtGallonsRange" onclick="event.stopPropagation()" onchange="loadDetailEstGallons(currentCustomerId)" style="padding:4px 8px;border:1px solid var(--border-input);border-radius:6px;font-size:12px;background:var(--bg-input);color:var(--text-primary);">
                        ...
                        <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();mgmtShowWaterSettingsModal()" title="Water source settings">&#9881;&#65039;</button>
                    </div>
                </div>
            ...
            <div class="card" id="detailPumpMonitorCard" style="display:none;">
                <div class="card-header" onclick="toggleCard('pump')" style="cursor:pointer;user-select:none;">
                    ...
                    <div style="display:flex;gap:6px;align-items:center;">
                        ...
                        <select id="mgmtPumpRange" onclick="event.stopPropagation()" onchange="loadDetailPumpMonitor(currentCustomerId)" style="padding:4px 8px;border:1px solid var(--border-input);border-radius:6px;font-size:12px;background:var(--bg-input,var(--bg-tile));color:var(--text-primary);">
                            <option value="24">Last 24 hours</option>
                            <option value="720" selected>Last 30 days</option>
                            <option value="2160">Last 90 days</option>
                            <option value="8760">Last year</option>
                        </select>
                        <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();mgmtShowPumpSettingsModal()" title="Pump settings">&#9881;&#65039;</button>
                    </div>
                </div>
        ...
        <div class="card">
            <div class="card-header" onclick="toggleCard('zones')" style="cursor:pointer;user-select:none;">
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <div id="mgmtAutoAdvanceToggle" onclick="event.stopPropagation()" style="display:none;"></div>
                </div>
            </div>
        ...
        <div class="card">
            <div class="card-header" onclick="toggleCard('history')" style="cursor:pointer;user-select:none;">
                ...
                <div style="display:flex;gap:6px;align-items:center;">
                    ...
                    <select id="mgmtHistoryRange" onclick="event.stopPropagation()" onchange="loadDetailHistory(currentCustomerId)" style="padding:4px 8px;border:1px solid var(--border-input);border-radius:6px;font-size:12px;background:var(--bg-input);color:var(--text-primary);">
                        <option value="24">Last 24 hours</option>
                        <option value="168">Last 7 days</option>
                        <option value="720">Last 30 days</option>
                        <option value="2160">Last 90 days</option>
                    ...
                    <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();mgmtExportHistoryCSV()">Export CSV</button>
                    <button class="btn btn-danger btn-sm" onclick="event.stopPropagation();mgmtClearRunHistory()">Clear History</button>
                </div>
            </div>
...
<div id="mgmtDynamicModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.4);z-index:10000;align-items:center;justify-content:center;" onclick="if(event.target===this)closeMgmtDynamicModal()">
    <div style="background:var(--bg-card);border-radius:12px;padding:0;width:90%;max-width:400px;max-height:80vh;box-shadow:0 8px 32px rgba(0,0,0,0.2);display:flex;flex-direction:column;">
        <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 20px 10px 20px;border-bottom:1px solid var(--border-light);">
            <h3 id="mgmtDynamicModalTitle" style="font-size:15px;font-weight:600;margin:0;color:var(--text-primary);"></h3>
            <button onclick="closeMgmtDynamicModal()" style="background:none;border:none;font-size:22px;cursor:pointer;color:var(--text-muted);padding:0 4px;">&times;</button>
        </div>
        <div id="mgmtDynamicModalBody" style="padding:12px 20px 20px 20px;overflow-y:auto;"></div>
    </div>
</div>
...
// --- Generic Collapsible Card System ---
var _mgmtCardLocks = {};
try { _mgmtCardLocks = JSON.parse(localStorage.getItem('flux_card_locks_mgmt') || '{}'); } catch(e) { _mgmtCardLocks = {}; }
function _saveMgmtCardLocks() { try { localStorage.setItem('flux_card_locks_mgmt', JSON.stringify(_mgmtCardLocks)); } catch(e) {} }

function toggleCard(key) {
    if (_mgmtCardLocks[key]) return; // locked open — ignore click
...
function renderCustomerGrid(customers) {
    ...
    grid.innerHTML = customers.map(c => {
        ...
        var issueBadgeHtml = '';
        if (issueCount > 0 && issueSummary && issueSummary.issues) {
            var sevCount = 0, annCount = 0, clarCount = 0;
            issueSummary.issues.forEach(function(iss) {
                if (iss.severity === 'severe') sevCount++;
                else if (iss.severity === 'annoyance') annCount++;
                else clarCount++;
            });
            if (sevCount > 0) issueBadgeHtml += '<span style="display:inline-block;margin-left:4px;padding:1px 7px;border-radius:8px;font-size:11px;font-weight:700;background:#e74c3c22;color:#e74c3c;" title="' + sevCount + ' severe">&#9888; ' + sevCount + '</span>';
            if (annCount > 0) issueBadgeHtml += '<span style="display:inline-block;margin-left:4px;padding:1px 7px;border-radius:8px;font-size:11px;font-weight:700;background:#f39c1222;color:#f39c12;" title="' + annCount + ' annoyance">&#9888; ' + annCount + '</span>';
            if (clarCount > 0) issueBadgeHtml += '<span style="display:inline-flex;align-items:center;gap:3px;margin-left:4px;padding:1px 7px;border-radius:8px;font-size:11px;font-weight:700;background:#3498db22;color:#3498db;" title="' + clarCount + ' clarification"><svg width="12" height="12" viewBox="0 0 16 16" style="flex-shrink:0;"><circle cx="8" cy="8" r="7" fill="none" stroke="#3498db" stroke-width="2"/><text x="8" y="12" text-anchor="middle" fill="#3498db" font-size="11" font-weight="bold" font-family="Arial,sans-serif">?</text></svg>' + clarCount + '</span>';
        }
...
async function initDetailMap(customer) {
    ...
    const addr = formatAddress(customer);
    if (!addr) {
        // Show message instead of silently hiding
        mapEl.style.display = 'block';
        mapEl.style.height = 'auto';
        mapEl.style.minHeight = '0';
        mapEl.innerHTML = '<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:12px;font-style:italic;background:var(--bg-tile);border-radius:8px;">Map unavailable — no address configured for this property</div>';
        return;
    }
    if (geocodeCache[addr]) {
    ...
    try {
        ...
        if (geo.lat !== null && geo.lon !== null) {
            ...
            mapEl.style.height = 'auto';
            mapEl.style.minHeight = '0';
            mapEl.innerHTML = '<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:12px;font-style:italic;background:var(--bg-tile);border-radius:8px;">Map unavailable — could not geocode address</div>';
        }
    } catch (e) {
        ...
        mapEl.style.height = 'auto';
        mapEl.style.minHeight = '0';
        mapEl.innerHTML = '<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:12px;font-style:italic;background:var(--bg-tile);border-radius:8px;">Map unavailable — geocoding error</div>';
    }
}
...
async function loadDetailData(id) {
    ...
    await loadDetailControls(id);  // Must complete before loadDetailZones — sets _zoneModes for pump/valve detection
    loadDetailZones(id);
...
async function loadDetailMoisture(id) {
    ...
    try {
        ...
        if (_mgmtLastMultiplierKey !== null && _mgmtLastMultiplierKey !== multKey) {
            console.log('[MOISTURE] Factors changed, refreshing schedule card');
            loadDetailControls(id);
        }
        _mgmtLastMultiplierKey = multKey;
            ...
            for (const [pid, probe] of Object.entries(probes)) {
                ...
                if (hasDeviceInfo) {
                    ...
                    if (devSensors.sleep_disabled) {
                        ...
                        html += '<button onclick="mgmtToggleSleepDisabled(\\'' + esc(pid) + '\\',' + (showDisabled ? 'false' : 'true') + ')" style="padding:1px 6px;font-size:10px;border:1px solid ' + btnColor + ';border-radius:4px;cursor:pointer;background:transparent;color:' + btnColor + ';">' + btnLabel + '</button>';
                        html += '<span id="mgmtSleepDisPending_' + esc(pid) + '" style="color:var(--color-warning);font-size:10px;' + (pendingSD != null ? '' : 'display:none;') + '">' + (pendingSD != null ? '⏳ Pending' : '') + '</span>';
                    }
                    if (devSensors.sleep_duration) {
                        ...
                        html += '<span id="mgmtSleepDurPending_' + esc(pid) + '" style="color:var(--color-warning);font-size:10px;' + (pendingSleep != null ? '' : 'display:none;') + '" title="Pending: will apply when probe wakes">' + (pendingSleep != null ? '⏳ Pending: ' + pendingSleep + ' min' : '') + '</span>';
                        html += '</div>';
                    }
...
async function mgmtToggleSleepDisabled(probeId, disabled) {
    try {
        const result = await api('/customers/' + currentCustomerId + '/moisture/probes/' + encodeURIComponent(probeId) + '/sleep-disabled', {
            method: 'PUT',
...
async function loadDetailZones(id) {
    ...
    try {
        ...
        try {
            ...
            window._mgmtZoneSprinklerCat = {};
            for (var eid2 in allHeads) {
                var catResult = getSprinklerCategories(allHeads[eid2].heads);
                if (catResult) window._mgmtZoneSprinklerCat[eid2] = catResult;
            }
        } catch(e) { window._mgmtZoneGpmMap = {}; window._mgmtZoneGpmShow = {}; window._mgmtZoneHeadCountShow = {}; window._mgmtZoneHeadCount = {}; window._mgmtZoneSprinklerCat = {}; }
        ...
        el.innerHTML = '<div class="tile-grid">' + zones.map(z => {
            ...
            <div class="tile ${isOn ? 'active' : ''}">
                ${(function() {
                    ...
                    var hasHeads = window._mgmtZoneHeadCountShow && window._mgmtZoneHeadCountShow[z.entity_id] && window._mgmtZoneHeadCount && window._mgmtZoneHeadCount[z.entity_id];
                    if (hasGpm || hasHeads) {
                        badges += '<div style="position:absolute;top:8px;right:10px;text-align:right;line-height:1.2;">';
                        if (hasGpm) {
                            badges += '<div><span style="font-size:14px;font-weight:700;color:var(--text-primary);">' + window._mgmtZoneGpmMap[z.entity_id].toFixed(1) + '</span><span style="font-size:10px;color:var(--text-secondary);margin-left:1px;">GPM</span></div>';
                        }
                        if (hasHeads) {
                            badges += '<div><span style="font-size:14px;font-weight:700;color:var(--text-primary);">' + window._mgmtZoneHeadCount[z.entity_id] + '</span><span style="font-size:10px;color:var(--text-secondary);margin-left:1px;">' + (window._mgmtZoneHeadCount[z.entity_id] === 1 ? 'head' : 'heads') + '</span></div>';
                        }
                        badges += '</div>';
                    }
                    return badges;
                ...
                <div class="tile-name">
                    ...
                    ${(function() {
                        ...
                        if (/pump|relay/.test(mv)) {
                            return '<span style="cursor:pointer;font-size:20px;color:var(--color-info);margin-left:4px;" onclick="event.stopPropagation();mgmtShowPumpSettingsModal()" title="Pump settings">&#9432;</span>';
                        }
                        return '<span style="cursor:pointer;font-size:20px;color:var(--color-info);margin-left:4px;" onclick="event.stopPropagation();mgmtShowZoneDetailsModal(\\'' + z.entity_id + '\\', decodeURIComponent(\\'' + encodeURIComponent(displayName) + '\\'))" title="Zone head details">&#9432;</span>';
                    })()}
                </div>
...
async function loadDetailControls(id) {
    ...
    try {
        ...
        if (window._pumpZoneEntity) {
            try {
                var ws = await api('/customers/' + id + '/water_settings?t=' + Date.now());
                if (!ws.water_source) {
                    ws.water_source = 'well';
                    ws.cost_per_1000_gal = 0;
                    await api('/customers/' + id + '/water_settings', { method: 'PUT', body: JSON.stringify(ws) });
                }
            } catch(e) {}
        }
        loadDetailPumpMonitor(id);
            ...
            if (pmZones.length > 0) {
                ...
                for (const pm of pmZones) {
                    ...
                    html += '<div class="tile">' +
                        ...
                        'onchange="mgmtZoneModeChanged(\\'' + id + '\\',\\'' + modeEid +
            ...
            if (hasMode) {
                if (mode) {
                    ...
                    html += '<td><select id="' + selId + '" style="padding:3px 6px;border:1px solid var(--border-input);border-radius:4px;font-size:12px;background:var(--bg-input);color:var(--text-primary);" ' +
                        'onchange="mgmtZoneModeChanged(\\'' + custId + '\\',\\'' + modeEid +
                        '\\',document.getElementById(\\'' + selId + '\\').value)">' +
...
async function mgmtZoneModeChanged(custId, modeEntityId, newMode) {
    ...
    if (aliases[zoneEid]) {
        ...
        window._currentZoneAliases = aliases;
        try {
            await api('/customers/' + custId + '/zone_aliases', {
                method: 'PUT',
                body: JSON.stringify({ zone_aliases: aliases }),
            });
        } catch(e) {}
    }
    setEntityValue(custId, modeEntityId, 'select', {option: newMode});
}
...
async function loadDetailEstGallons(id) {
    ...
    try {
        ...
        try { waterSettings = await api('/customers/' + id + '/water_settings?t=' + Date.now()); } catch(e) {}
...
async function mgmtShowWaterSettingsModal() {
    var id = currentCustomerId;
    if (!id) return;
    var ws = {};
    try { ws = await api('/customers/' + id + '/water_settings?t=' + Date.now()); } catch(e) {}

    var body = '<div style="display:flex;flex-direction:column;gap:12px;">';

    body += '<div><label style="font-size:13px;font-weight:600;display:block;margin-bottom:4px;">Water Source</label>' +
        '<select id="mgmtWaterSourceType" onchange="mgmtToggleWaterCostField()" style="width:100%;padding:8px;border:1px solid var(--border-input);border-radius:6px;font-size:14px;background:var(--bg-input);color:var(--text-primary);box-sizing:border-box;">' +
    ...
    body += '<div id="mgmtWaterCostRow" style="' + (showCost ? '' : 'display:none;') + '">' +
        '<label style="font-size:13px;font-weight:600;display:block;margin-bottom:4px;">Cost per 1,000 Gallons ($)</label>' +
        '<input type="number" id="mgmtWaterCostPer1000" value="' + (ws.cost_per_1000_gal || '') + '" step="0.01" min="0" style="width:100%;padding:8px;border:1px solid var(--border-input);border-radius:6px;font-size:14px;background:var(--bg-input);color:var(--text-primary);box-sizing:border-box;" placeholder="e.g. 5.50">' +
        '</div>';

    body += '<div id="mgmtWellWaterNote" style="' + (ws.water_source==='well' ? '' : 'display:none;') +
    ...
    body += '<div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;">' +
        '<button class="btn btn-secondary" onclick="closeMgmtDynamicModal()">Cancel</button>' +
        '<button class="btn btn-primary" onclick="mgmtSaveWaterSettings()">Save</button>' +
        '</div>';
    body += '</div>';

    mgmtShowModal('\\u{1F4A7} Water Source Settings', body, '420px');
}

function mgmtToggleWaterCostField() {
    var src = document.getElementById('mgmtWaterSourceType').value;
    document.getElementById('mgmtWaterCostRow').style.display = (src === 'city' || src === 'reclaimed') ? '' : 'none';
    document.getElementById('mgmtWellWaterNote').style.display = (src === 'well') ? '' : 'none';
}

async function mgmtSaveWaterSettings() {
    var id = currentCustomerId;
    if (!id) return;
    var payload = {
        water_source: document.getElementById('mgmtWaterSourceType').value,
        cost_per_1000_gal: parseFloat(document.getElementById('mgmtWaterCostPer1000').value) || 0
    };
    try {
        await api('/customers/' + id + '/water_settings', { method: 'PUT', body: JSON.stringify(payload) });
        closeMgmtDynamicModal();
...
async function loadDetailIssues(custId) {
    ...
    const body = document.getElementById('cardBody_issues');
...
<p style="margin-bottom:10px;">The Estimated Gallons card shows total water usage calculated from zone run times and configured GPM values. Click the <strong>&#9881;&#65039; gear</strong> button to configure the customer&rsquo;s water source:</p>
<ul style="margin:4px 0 12px 20px;"><li style="margin-bottom:4px;"><strong>City Water</strong> &mdash; Municipal water supply; enter cost per 1,000 gallons to see estimated water costs</li><li style="margin-bottom:4px;"><strong>Reclaimed Water</strong> &mdash; Recycled/reclaimed water; enter cost per 1,000 gallons</li><li style="margin-bottom:4px;"><strong>Well Water</strong> &mdash; Private well (auto-detected when a Pump Start Relay is configured); no water utility cost &mdash; electricity costs are tracked in the Pump Monitor card</li></ul>
...
<div style="background:var(--bg-tile);border-radius:6px;padding:8px 12px;margin:8px 0 12px 0;font-size:13px;">&#128161; If the customer&rsquo;s controller has a Pump Start Relay zone, the water source automatically defaults to Well Water. You can change this at any time via the settings gear.</div>

<h4 style="font-size:15px;font-weight:600;color:var(--text-primary);margin:20px 0 8px 0;">&#9889; Pump Monitor</h4>
<p style="margin-bottom:10px;">When a customer&rsquo;s controller has a zone configured as a <strong>Pump Start Relay</strong>, a dedicated Pump Monitor card appears alongside the Estimated Gallons card in the property detail view. The card shows:</p>
<ul style="margin:4px 0 12px 20px;"><li style="margin-bottom:4px;"><strong>Total Cycles</strong> &mdash; Number of completed pump on/off cycles in the selected time range</li><li style="margin-bottom:4px;"><strong>Run Hours</strong> &mdash; Total hours the pump has been running</li><li style="margin-bottom:4px;"><strong>Power Used</strong> &mdash; Total electricity consumption in kWh, calculated from pump HP/kW and run time</li><li style="margin-bottom:4px;"><strong>Estimated Cost</strong> &mdash; Electricity cost based on the configured rate ($/kWh)</li></ul>
<p style="margin-bottom:10px;">Use the <strong>time range dropdown</strong> to filter: Last 24 Hours, Last 30 Days (default), Last 90 Days, or Last Year. Click the <strong>&#9881;&#65039; gear</strong> button to configure pump settings:</p>
<ul style="margin:4px 0 12px 20px;"><li style="margin-bottom:4px;"><strong>Brand</strong> &mdash; Pump manufacturer (e.g., Pentair, Hayward)</li><li style="margin-bottom:4px;"><strong>Horsepower / Kilowatts</strong> &mdash; Enter either value and the other auto-calculates (1 HP = 0.7457 kW)</li><li style="margin-bottom:4px;"><strong>Voltage</strong> &mdash; Operating voltage (default 240V)</li><li style="margin-bottom:4px;"><strong>Year Installed</strong> &mdash; Pump age is calculated automatically and displayed on the card</li><li style="margin-bottom:4px;"><strong>Electricity Cost / Peak Rate</strong> &mdash; Standard and peak hour electricity rates ($/kWh)</li></ul>
...
async function mgmtShowZoneDetailsModal(entityId, displayName) {
    ...
    try {
        var resp = await api('/customers/' + custId + '/zone_heads/' + entityId + '?t=' + Date.now());
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            ...
            else if (document.getElementById('mgmtDynamicModal').style.display === 'flex') closeMgmtDynamicModal();
