...
- **Apply Factors to Schedule — "applied to 0 zones"** — Duration adjustment now iterates all captured base duration entities directly instead of requiring a zone-entity-to-duration-entity name match, which failed when naming conventions differed
- **Apply Factors toggle flashing** — The toggle tile is now in a stable DOM container outside the Schedule card rebuild cycle and uses targeted updates; no longer flickers on every 30-second refresh
- **Watering Factor tile not updating after settings changes** — The System Status card (Watering Factor tile) now refreshes immediately after saving weather rules, evaluating weather, saving moisture settings, or toggling Apply Factors; previously it only updated on the next periodic refresh
