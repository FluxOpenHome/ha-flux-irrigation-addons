...
- **Probe Auto-Detection** — New `/devices/{id}/autodetect` API endpoint automatically maps Gophr device sensors to depth assignments (sensor 3 → shallow, sensor 2 → mid, sensor 1 → deep) and detects WiFi, battery, and sleep duration sensors from entity names; eliminates manual sensor selection
