...
- **Zone head details** — New zone details modal (click the ℹ icon on any zone tile) lets you document every sprinkler head in a zone. For each head: head type (pop-up spray, rotary nozzle, gear rotor, impact rotor, micro-spray, bubbler, drip emitter, drip line, fixed spray, strip nozzle), brand (Rain Bird, Hunter, Toro, K-Rain, Irritrol, Orbit, etc.), model from a filtered picklist of 65+ real sprinkler models with GPM auto-fill, mount type (pop-up, stationary, riser, shrub, on-grade), spray arc in degrees, throw radius in feet, pop-up height, operating PSI, and a name/location label. Total zone GPM is calculated automatically and displayed prominently on each zone card. "Show GPM on card" checkbox controls per-zone visibility. Model database stored in `sprinkler_models.json` for easy editing. Data persists in `/data/zone_nozzle_details.json`. Available on both homeowner and management dashboards.
- **Next wake time on probe cards** — When a Gophr moisture probe is sleeping, the probe card now shows the estimated next wake time instead of just "😴 Sleeping". Displays as "Wake in X min" (under 60 minutes), "Wake at HH:MM" (over 60 minutes), or "Waking soon" (imminent or overdue). Calculated from the status LED's `last_changed` timestamp plus the sleep duration. Available on both homeowner and management dashboards.
...
- **Weather factor not updating schedule durations during active runs** — When the weather-based watering factor changed while zones were actively running, `apply_adjusted_durations()` silently refused to update (safety guard against mid-run changes). The new factor was lost until the next periodic evaluation. Now, a deferred flag is set when the update is blocked, and when the last running zone turns off, the factor re-application is triggered automatically.
- **Zones 17-32 visible in schedule card when only 16 zones detected** — The schedule card (zone enables, durations, modes) showed all 32 pre-created zone entities even when only 16 physical zones were detected via expansion boards. This was caused by a race condition: `loadSensors()` and `loadControls()` ran in parallel, so `_detectedZoneCount` wasn't set yet when the schedule card rendered. Fixed by awaiting `loadSensors()` before `loadControls()` on both dashboards.
